...
static int cb_read(void *const stream, unsigned char *const ptr, int const nbytes) {
  ...
  size_t read = 0;
  error err = ovl_file_read(f, ptr, (size_t)nbytes, &read);
  if (efailed(err)) {
    return -1;
  }
  return (int)read;
...
static struct test_util_decoded_audio decoder_all(wchar_t const *const filepath) {
  ...
  err = OV_ARRAY_GROW(&result.buffer, head->channel_count);
  if (efailed(err)) {
    goto cleanup;
  }
  ...
  err = OV_ARRAY_GROW(&temp, channels * decoder_block);
  if (efailed(err)) {
    goto cleanup;
  }
  size_t read = 0;
...
static void all(void) {
  ...
  err = ovl_source_file_create(TESTDATADIR NSTR("/test.opus"), &source);
  if (efailed(err)) {
    goto cleanup;
  }
  err = ovl_audio_decoder_opus_create(source, &d);
  ...
  file_all = test_util_create_wave_file(NSTR("opus-all.wav"), (size_t)info->samples, info->channels, info->sample_rate);
