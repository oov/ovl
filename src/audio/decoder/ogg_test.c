...
static size_t cb_read(void *const ptr, size_t const size, size_t const nmemb, void *const datasource) {
  ...
  if (!ovl_file_read(f, ptr, size * nmemb, &read, &err)) {
    return 0;
  }
  return read / size;
}
...
static struct test_util_decoded_audio decoder_all(wchar_t const *const filepath) {
  ...
  OggVorbis_File vf;
  {
    ...
    if (!OV_ARRAY_GROW(&result.buffer, (size_t)vi->channels)) {
      OV_ERROR_SET_GENERIC(&err, ov_error_generic_out_of_memory);
      goto cleanup;
    }
    result.buffer[0] = NULL;
    if (!OV_ARRAY_GROW(&result.buffer[0], (size_t)samples * (size_t)vi->channels)) {
      OV_ERROR_SET_GENERIC(&err, ov_error_generic_out_of_memory);
      goto cleanup;
    }
...
static void all(void) {
...

  {
    ...
    if (!TEST_SUCCEEDED(ovl_audio_decoder_ogg_create(source, &d, &err), &err)) {
      goto cleanup;
    }
    ...
    file_all =
        test_util_create_wave_file(NSTR("ogg-all.wav"), (size_t)info->samples, info->channels, info->sample_rate);
