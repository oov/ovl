...
static size_t cb_read(void *buf, size_t size, void *user_data) {
  struct ovl_file *f = (struct ovl_file *)user_data;
  size_t read;
  struct ov_error err = {0};
  if (!ovl_file_read(f, buf, size, &read, &err)) {
...
static int cb_seek(uint64_t position, void *user_data) {
  ...
  struct ov_error err = {0};
  if (!ovl_file_seek(f, (int64_t)position, ovl_file_seek_method_set, &err)) {
    OV_ERROR_REPORT(&err, NULL);
    return 1;
  }
  return 0;
...
static struct test_util_decoded_audio decoder_all(wchar_t const *const filepath) {
...

  {
    ...
    if (!OV_ARRAY_GROW(&result.buffer, channels)) {
...
static void all(void) {
  ...
  bool success = false;
  {
    ...
    file_all =
        test_util_create_wave_file(NSTR("mp3-all.wav"), (size_t)info->samples, info->channels, info->sample_rate);
