...
static bool test_single_format(struct test_case const *const test) {
...

  {
    ...
    ov_snprintf_wchar(filename, 512, NULL, NSTR("%s.wav"), test->name);
...
static bool test_single_info(struct test_case const *const test) {
  struct ovl_audio_decoder *d = NULL;
  struct ovl_source *source = NULL;
  struct ov_error err = {0};
  bool success = false;
...
static void various_formats(void) {
  static struct test_case const tests[] = {
