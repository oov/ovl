...
static size_t utf16_to_utf8(
  ...
  char16_t *const work = (char16_t *)(void *)(buf + offset);
  uint8_t const *const p = (uint8_t *)(void *)work;
  if (isle) {
...
static NODISCARD bool read_txxx_frame(struct ovl_audio_tag *const loop_info,
...

  {
    if (frame_size < 2) { // encoding + null terminator
      OV_ERROR_SET(err, ov_error_type_generic, ov_error_generic_fail, gettext("TXXX frame too small"));
      goto cleanup;
    }

    uint8_t encoding;
    if (ovl_source_read(source, &encoding, offset, 1) != 1) {
      OV_ERROR_SET(err, ov_error_type_generic, ov_error_generic_fail, gettext("Failed to read TXXX encoding"));
      goto cleanup;
    }
    ++offset;
    ...
    if (encoding != id3v2_encoding_iso_8859_1 && encoding != id3v2_encoding_utf_16_with_bom &&
      ...
      OV_ERROR_SETF(
          err, ov_error_type_generic, ov_error_generic_fail, "%d", gettext("Unsupported TXXX encoding: %d"), encoding);
      goto cleanup;
    }
    ...
    if (ovl_source_read(source, buf, offset, content_size) < content_size) {
      OV_ERROR_SET(err, ov_error_type_generic, ov_error_generic_fail, gettext("Failed to read TXXX content"));
      goto cleanup;
    }
    ...
    if (encoding == id3v2_encoding_utf_16_with_bom || encoding == id3v2_encoding_utf_16be_without_bom) {
      for (size_t i = 0; i < content_size; i += 2) {
        if (buf[i] == 0 && buf[i + 1] == 0) {
          desc = u8buf;
          desc_len = to_utf8(buf, i, (enum id3v2_encoding)encoding, desc, u8_buffer_size);
          value = u8buf + desc_len + 1;
          value_len = to_utf8(
              buf + i + 2, content_size - i - 2, (enum id3v2_encoding)encoding, value, u8_buffer_size - desc_len - 1);
    ...
    if (desc_len == SIZE_MAX || value_len == SIZE_MAX) {
      OV_ERROR_SET(err,
                   ...
                   gettext("The description/value separator not found in TXXX frame"));
      goto cleanup;
    }
...
static NODISCARD bool read_text_frame(struct ovl_audio_tag *const tag,
                                      struct ovl_source *const source,
                                      uint64_t offset,
                                      uint32_t frame_size,
                                      const char *frame_id,
  ...
  if (frame_size < 1) {
    OV_ERROR_SET(err, ov_error_type_generic, ov_error_generic_fail, gettext("Text frame too small"));
    return false;
  }
  if (frame_size - 1 > buffer_max_len - 1) {
    return true; // Text frame too large but not an error
  }
...

  {
    ...
    if (read < 1) {
      OV_ERROR_SETF(
          err, ov_error_type_generic, ov_error_generic_fail, "%s", gettext("Failed to read %s encoding"), frame_id);
      goto cleanup;
    }
    offset += read;
    ...
    if (encoding != id3v2_encoding_iso_8859_1 && encoding != id3v2_encoding_utf_16_with_bom &&
      ...
      OV_ERROR_SETF(err,
                    ...
                    ov_error_generic_fail,
                    "%s%d",
                    gettext("Unsupported %s encoding: %d"),
                    frame_id,
                    encoding);
      goto cleanup;
    }
    ...
    if (read < content_size) {
      OV_ERROR_SETF(
          err, ov_error_type_generic, ov_error_generic_fail, "%s", gettext("Failed to read %s content"), frame_id);
      goto cleanup;
    }
    offset += read;
    if (encoding == id3v2_encoding_utf_16_with_bom || encoding == id3v2_encoding_utf_16be_without_bom) {
      char out[buffer_max_len];
      size_t const conv = to_utf8(buf, content_size, (enum id3v2_encoding)encoding, out, buffer_max_len);
      if (conv == 0) {
        goto cleanup; // Invalid sequence or buffer too small but not an error
      }
      content_size = conv;
    ...
    if (strcmp(frame_id, "TIT2") == 0) {
      if (!ovl_audio_tag_set_title(tag, buf, content_size, err)) {
        OV_ERROR_ADD_TRACE(err);
        goto cleanup;
      }
    } else if (strcmp(frame_id, "TPE1") == 0) {
      if (!ovl_audio_tag_set_artist(tag, buf, content_size, err)) {
        OV_ERROR_ADD_TRACE(err);
        goto cleanup;
      }
    }
...
NODISCARD bool ovl_audio_tag_id3v2_read(struct ovl_audio_tag *const tag,
                                        struct ovl_source *const source,
                                        uint64_t offset,
...

  {
    ...
    if (read < sizeof(header)) {
      OV_ERROR_SET(err, ov_error_type_generic, ov_error_generic_fail, gettext("Failed to read ID3 header"));
      goto cleanup;
    }
    offset += read;
    ...
    if (header.flags & 0x40) { // has extended header
      ...
      if (read < sizeof(ext_size_bytes)) {
        OV_ERROR_SET(err, ov_error_type_generic, ov_error_generic_fail, gettext("Failed to read extended header"));
        goto cleanup;
      }
      offset += read;
    ...
    if (header.version_major == 4) {
      ...
      if (ext_size < 6 || offset + ext_size > chunk_end) {
        OV_ERROR_SET(err, ov_error_type_generic, ov_error_generic_fail, gettext("Invalid extended header size"));
        goto cleanup;
      }
      offset += ext_size;
    }
    ...
    uint32_t const frame_size = ((uint32_t)frame.size[0] << 24) |
      ...
      if (memcmp(frame_id, "TIT2", 4) == 0 || memcmp(frame_id, "TPE1", 4) == 0) {
        if (!read_text_frame(&t, source, offset, frame_size, frame_id, err)) {
          OV_ERROR_ADD_TRACE(err);
          goto cleanup;
        }
      } else if (memcmp(frame_id, "TXXX", 4) == 0) {
        if (!read_txxx_frame(&t, source, offset, frame_size, err)) {
          OV_ERROR_ADD_TRACE(err);
          goto cleanup;
        }
      }
